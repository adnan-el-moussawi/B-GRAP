...
public abstract class MessageWrapper<I extends WritableComparable, M extends Writable>
	...
	/**
	 * Subclasses must provide the vertex Id class type appropriately and can use
	 * getVertexIdClass() to do it.
	 *
	 * @return Class
	 */
	...
	/**
	 * Subclasses must provide the message class type appropriately and can use
	 * getMessageClass() to do it.
	 *
	 * @return Class<M>
	 */
	...
	public String toString() {
		return "MessageWrapper{" + ", sourceId=" + sourceId + ", message=" + message + '}';
	}
	...
	public final int compareTo(final MessageWrapper<I, M> wrapper) {
		...
		if (((Comparable<I>) sourceId).compareTo((I) wrapper.getSourceId()) == 0) {
			return ((Comparable<M>) message).compareTo((M) wrapper.getMessage());
		} else {
			return ((Comparable<I>) sourceId).compareTo((I) wrapper.getSourceId());
		}
	}
