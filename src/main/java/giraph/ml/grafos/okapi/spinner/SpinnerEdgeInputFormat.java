...
public class SpinnerEdgeInputFormat extends TextEdgeInputFormat<LongWritable, BGRAP_EdgeValue> {
	/** Splitter for endpoints */
	private static final Pattern SEPARATOR = Pattern.compile("[\001\t ]");
	// private static final Pattern SEPARATOR = Pattern.compile("[\t ]");

	@Override
	public EdgeReader<LongWritable, BGRAP_EdgeValue> createEdgeReader(InputSplit split, TaskAttemptContext context)
			throws IOException {
	...
	public class SpinnerEdgeReader extends TextEdgeReaderFromEachLineProcessed<String[]> {
		...
		@Override
		protected BGRAP_EdgeValue getValue(String[] endpoints) throws IOException {
			BGRAP_EdgeValue value = new BGRAP_EdgeValue();
