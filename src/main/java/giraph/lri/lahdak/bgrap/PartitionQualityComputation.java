...
public class PartitionQualityComputation extends BGRAP_Partitionning {
	...
	public static class AddVirtualEdges
		...
		public void compute(Vertex<LongWritable, BGRAP_VertexValue, BGRAP_EdgeValue> vertex, Iterable<LongWritable> messages)
			...
			for (LongWritable other : messages) {
				BGRAP_EdgeValue edgeValue = vertex.getEdgeValue(new LongWritable(other.get()));
			...
			BGRAP_VertexValue value = vertex.getValue();
	...
	public static class PartitionerMasterCompute extends SuperPartitionerMasterCompute {
		...
		public void initialize() throws InstantiationException, IllegalAccessException {
			...
			super.defineAdditionalAggregators();
		}
		...
		protected void saveRealStats() {
			...
			try {
				...
				file.write(String.format("%.3f", realEdgeMaxNormLoad) + ";");
				file.write(String.format("%.3f", realEdgeMaxMinLoad) + ";");
		...
		protected void saveVirtualStats() {
			...
			try {
				...
				file.write(getConf().getMaxWorkers() + ";");
				file.write(numberOfPartitions + repartition + ";");
				file.write(String.format("%.3f", virtualEdgeMaxNormLoad) + ";");
				file.write(String.format("%.3f", virtualEdgeMaxMinLoad) + ";");
	...
	public static class OutputFormat<I extends LongWritable, V extends Writable, E extends Writable>
		...
		protected class VertexValueWriter extends TextVertexWriterToEachLine {
			...
			protected Text convertVertexToLine(Vertex<I, V, E> vertex) throws IOException {
				...
				BGRAP_VertexValue v = (BGRAP_VertexValue) vertex.getValue();
	...
	public static class IntIdEmptyOutputFormat<I extends IntWritable, V extends Writable, E extends Writable>
			extends TextVertexOutputFormat<I, V, E> {
		/** Specify the output delimiter */
		public static final String LINE_TOKENIZE_VALUE = "output.delimiter";
		/** Default output delimiter */
	...
	public static class LongEmptyOutputFormat<I extends LongWritable, V extends Writable, E extends Writable>
			extends TextVertexOutputFormat<I, V, E> {
		/** Specify the output delimiter */
		public static final String LINE_TOKENIZE_VALUE = "output.delimiter";
		/** Default output delimiter */
	...
	public static class InputFormat extends TextVertexInputFormat<LongWritable, BGRAP_VertexValue, BGRAP_EdgeValue> {
		...
		public class SpinnerLongDoubleDoubleVertexReader extends TextVertexReaderFromEachLineProcessed<String[]> {
			...
			protected BGRAP_VertexValue getValue(String[] data) throws IOException {
				return new BGRAP_VertexValue(partition, partition);
			}

			@Override
			protected List<Edge<LongWritable, BGRAP_EdgeValue>> getEdges(String[] tokens) throws IOException {
				List<Edge<LongWritable, BGRAP_EdgeValue>> edges = Lists.newArrayListWithCapacity(tokens.length - 1);
				for (int n = 1; n < tokens.length; n++) {
					edges.add(EdgeFactory.create(
							new LongWritable(Long.parseLong(tokens[n])), new BGRAP_EdgeValue( (short)-1, (byte) 2, false)));
