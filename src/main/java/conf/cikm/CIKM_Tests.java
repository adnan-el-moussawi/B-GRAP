 ...
 *
 */
public class CIKM_Tests extends GiraphClusterConfiguration {
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new CIKM_Tests(), data);
	}
	...
	String[] balanceAlgorithms = new String[] { "bgrap.VERTEX_BALANCE", "bgrap.EDGE_BALANCE" };
	String[] initAlgorithms = new String[] { "BGRAP_init",
			// "InitializeSampleGD",
			"InitializeSampleHD", "InitializeSampleRD",
			// "InitializeSampleCC",
	};
...
//					"Flixster",32
	...
	public int run(String[] argDataNames) {
		...
		setWorkersNumber(8);
		setCoresNumber(8);
		setMaxMemoryByContainer(32 * 1024);
		/*
		...
		// graph 500
		//int j, d, maxK = 64, maxRun = 10, runStart = 7, kStart = 2, dStart = 0, bAlgoStart = 1, initAlgoStart = 0;
		//int j, d, maxK = 64, maxRun = 10, runStart = 0, kStart = 16, dStart = 0, bAlgoStart = 1, initAlgoStart = 0;
		// Twitter
		//int j, d, maxK = 32, maxRun = 10, runStart = 6, kStart = 8, dStart = 0, bAlgoStart = 1, initAlgoStart = 0;
		int j, d, maxK = 32, maxRun = 10, runStart = 1, kStart = 2, dStart = 0, bAlgoStart = 0, initAlgoStart = 0;
		kappa = 0.5f;
		dStart = 0;
				...
				for (d = dStart; d < argDataNames.length; d++) {
					for (k = kStart; k <= maxK; k = k * 2) {
						...
						for (int run = runStart; run <= maxRun; run++) {
							...
							try {
								updateRessourcesForGiraphOnYarn("tipi59", "hdfs://tipi59:9099",
										getMaxMemoryByContainer(), getCoresNumber());
								...
								if (flag) {
									...
									if (filename.equals("Twitter") || filename.equals("Graph500")
											|| filename.equals("Orkut") || filename.equals("SinaWeibo")) {
								...
								if(restart){
									System.out.println("[ERROR] Catch IOException in run (" + run + ").");
									System.out.println("Try to Restart HADOOP CLUSTER");
